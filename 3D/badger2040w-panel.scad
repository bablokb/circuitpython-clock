// -----------------------------------------------------------------------------
// 3D-Model (OpenSCAD) for a Badger2040W.
//
...
module plate() {
  difference() {
    cuboid([x1_panel,y1_panel,z_panel],
            ...
            edges="Z");
    translate([-(x1_panel-x2_panelw)/2+rim-gap,0,-fuzz])
      cuboid([x2_panelw,y2_panel,z2_panel+fuzz],
  ...
  difference() {
    ...
    translate([0,0,fuzz])
            cuboid([x_wall-2*w2,y_wall-2*w2,z_wall+2*fuzz],
                   rounding=r_panel, anchor=TOP+CENTER,
                   edges="Z");
    // cutout right side (micro-USB)
    translate([x_wall/2,-y1_panel/2+y_musb_off,fuzz])
             cuboid([rim+fuzz,y_musb,z_wall+2*fuzz],anchor=TOP+CENTER);
    // cutout left side
    translate([-x_wall/2,0,fuzz])
             cuboid([rim+fuzz,y_panel_cutout,z_wall+2*fuzz],anchor=TOP+CENTER);
    // cutout back-buttons
